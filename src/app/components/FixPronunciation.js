...
export default function FixPronunciation({ close, resetTask }) {
    ...
    return (
        <div className="fixed left-0 top-0 h-screen w-screen backdrop-blur bg-white/20 flex items-center justify-center z-10">
            <form className="bg-black max-w-lg w-full p-4 mx-4" onSubmit={handleSubmit} ref={formRef}>
                <h2 className="text-2xl">Fix Pronunciation</h2>
                <div className="my-8">
                    <div className="w-full my-4">
                        <div className="text-slate-400">Hi</div>
                        <input className="bg-transparent border w-full border-slate-500 px-2 py-1" name="hi" type="text" autoComplete="off" pattern="[\u0900-\u097F\s]+" required/>
                    </div>
                    <div className="w-full my-4">
                        <div className="text-slate-400">Pronunciation</div>
                        <input className="bg-transparent border w-full border-slate-500 px-2 py-1" name="en_transliteration" type="text" autoComplete="off" pattern="[A-Za-z]+" required/>
                    </div>
                    <div className="flex gap-2">
                        <input id="ignore" name="ignore" type="checkbox"/>
